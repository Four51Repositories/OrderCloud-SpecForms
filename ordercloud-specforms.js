...
angular.module('OrderCloud-SpecForms')
    .directive('custommaskfield', custommaskfield)
    .directive('mask', mask)
    .directive('customcasefield', customcasefield)
;

function custommaskfield() {
    var directive = {
        scope: {
            customfield : '=',
            changed: '=',
        ...
        return [
            '<div class="view-form-icon" ng-class="{\'view-form-icon-input-group\':((customfield.Prefix && !hideprefix) || (customfield.Suffix && !hidesuffix))}">',
                '<div ng-if="customfield.Lines <= 1">',
                    '<label>{{customfield.Label || customfield.Name | r}}</label>',
...
function customcasefield() {
    var directive = {
        ...
        restrict: 'E',
        template: template,
        controller: CustomCaseFieldCtrl
    };
    return directive;
    ...
    CustomCaseFieldCtrl.$inject = ['$scope'];
    function CustomCaseFieldCtrl($scope) {
